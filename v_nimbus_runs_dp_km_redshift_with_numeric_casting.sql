-- ============================================================================
-- ALTERNATIVE VERSION: With Numeric Type Casting (Like MV_RUNS_DP_KM table)
-- ============================================================================
-- This version casts the statistics fields to numeric types (float/int)
-- and handles 'NaN' and empty string values using NULLIF
-- ============================================================================
...
ns AS (
    SELECT
        ...
        -- Cast statistics fields to numeric types with NULLIF for 'NaN' and empty strings
        NULLIF(NULLIF(JSON_EXTRACT_PATH_TEXT(nimbus_steps.metadata_json, 'fractional_factor', true), 'NaN'), '')::float AS fractional_factor,
        ...
        NULLIF(NULLIF(JSON_EXTRACT_PATH_TEXT(nimbus_steps.metadata_json, 'statistics', 'executor_cores', true), 'NaN'), '')::int AS executor_cores,
        NULLIF(NULLIF(JSON_EXTRACT_PATH_TEXT(nimbus_steps.metadata_json, 'statistics', 'driver_cpu_hours', true), 'NaN'), '')::float AS driver_cpu_hours,
        NULLIF(NULLIF(JSON_EXTRACT_PATH_TEXT(nimbus_steps.metadata_json, 'statistics', 'app_required_cores', true), 'NaN'), '')::float AS app_required_cores,
        NULLIF(NULLIF(JSON_EXTRACT_PATH_TEXT(nimbus_steps.metadata_json, 'statistics', 'duration_in_minutes', true), 'NaN'), '')::float AS step_app_duration_in_minutes
